...
local function set_attach()
    vim.api.nvim_create_autocmd("LspAttach", {
        ...
        callback = function(args)
            ...
            vim.keymap.set("n", M.options.keymaps.declaration, function() vim.lsp.buf.declaration() end, { buffer = 0 })
        end,
    })
end

local function better_gf()
    -- Fix for using gf on paths with "res://"
    vim.api.nvim_create_autocmd("BufReadCmd", {
        pattern = "res://*",
        callback = function(args)
            -- Get full buffer path
            local file_uri = args.file
            -- Strip res:// prefix
            local path = file_uri:gsub("^res://*", "")
            -- Force close current buffer and open the path as a normal file
            vim.schedule(function()
                vim.cmd("bd!")                               -- Close the current buffer
                vim.cmd("edit " .. vim.fn.fnameescape(path)) -- Open the real file
            end)
        end
    })
end
